...
*/
              ...
              g_object_get(video_sink->gobj(), "widget", &video_widget_c, NULL);

              if (video_widget_c) {
                // Wrap the raw GtkWidget* into a Gtk::Widget*
                Gtk::Widget* bvw->video_widget = Glib::wrap(video_widget_c);

                // Show the widget, as in the C code
                video_widget->show();

                // Add the video widget to the stack and set it as the visible child
                stack->add(*video_widget, "video");
...
//  Adds a %NULL-terminated list of elements to a bin
              ...
              g_signal_connect(bvw->decodebin, "pad-added", G_CALLBACK(on_padon_pad_added), bvw);
