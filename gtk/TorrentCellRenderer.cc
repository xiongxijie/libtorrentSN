    ...
    Gtk::CellRendererState flags)
{
    ...
    auto color = Gdk::RGBA();
    if (torrent.get_error_code() && (flags & TR_GTK_CELL_RENDERER_STATE(SELECTED)) == Gtk::CellRendererState{} &&
    ...
    Gtk::CellRendererState flags)
{
    ...
    /*Progress Bar*/
    auto const progress_color = get_progress_bar_color(torrent, widget);
    /* 5.35MB of 1.06GB (0.51%), uploaded None */
    auto const gstr_prog = torrent.get_long_progress_text();
    /* Downloading from x of y connected peers - 2.56MB/s▼ 49KB/s▲ */
    auto const gstr_stat = torrent.get_long_status_text();
    renderer_.get_padding(xpad, ypad);
