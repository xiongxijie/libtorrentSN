...
class MessageLogWindow::Impl
{
    ...
    private:
        ...
        void level_combo_init(Gtk::ComboBox* level_combo);
    ...
    private:
        ...
        tr_log_level maxLevel_ = tr_log_level::TR_LOG_INFO;
        bool isPaused_ = false;
        sigc::connection refresh_tag_;

    static auto inline const level_names_ = std::array<std::pair<tr_log_level, char const*>, 5U>{ {
        { tr_log_level::TR_LOG_CRITICAL, C_("Logging level", "Critical") },
        { tr_log_level::TR_LOG_ERROR, C_("Logging level", "Error") },
        { tr_log_level::TR_LOG_WARN, C_("Logging level", "Warning") },
        { tr_log_level::TR_LOG_INFO, C_("Logging level", "Information") },
        { tr_log_level::TR_LOG_DEBUG, C_("Logging level", "Debug") },
    } };
};
...
{
    auto const pref_level = static_cast<tr_log_level>(core_->get_sett().get_int(lt::settings_pack::TR_KEY_message_level));
    auto const default_level = tr_log_level::TR_LOG_INFO;
...
void MessageLogWindow::Impl::level_combo_changed_cb(Gtk::ComboBox* combo_box)
{
    ...
    core_->set_pref(lt::settings_pack::TR_KEY_message_level, level);
    maxLevel_ = level;
    filter_->refilter();
...
/*Save As , Export settings to local File*/
...
void setForegroundColor(Gtk::CellRendererText* renderer, tr_log_level level)
{
    switch (level)
    {
    case tr_log_level::TR_LOG_CRITICAL:
    case tr_log_level::TR_LOG_ERROR:
    case tr_log_level::TR_LOG_WARN:
        renderer->property_foreground() = "red";
        break;

    case tr_log_level::TR_LOG_DEBUG:
    case tr_log_level::TR_LOG_TRACE:
...
tr_log_message* addMessages(Glib::RefPtr<Gtk::ListStore> const& store, tr_log_message* head)
{
    ...
    while (head != nullptr && head->next != nullptr)
    {
        ...
        if (message.level == tr_log_level::TR_LOG_ERROR)
        {
    ...
    if (!isPaused_)
    {
        if (auto* msgs = tr_logGetQueue(); msgs != nullptr)
        {
            ...
            tr_log_message* tail = addMessages(store_, msgs);
                        // std::cout << "tail->message " << tail->message << std::endl;
            if (myTail != nullptr)
            {
                myTail->next = msgs;
            }
...
{
                        std::cout << "MessageLogWindow::~MessageLogWindow()  " << std::endl;
}


MessageLogWindow::Impl::~Impl()
{
                        std::cout << "MessageLogWindow::Impl::~Impl() " << std::endl;
          ...
          SECONDARY_WINDOW_REFRESH_INTERVAL_SECONDS))
{
    ...
    maxLevel_ = static_cast<tr_log_level>(core_->get_sett().get_int(lt::settings_pack::TR_KEY_message_level));
