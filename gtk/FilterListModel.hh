...
/*
ItemT is Torrent actually
FilterType is TorrentFilter acutally
...
FilterListModel<ItemT>::FilterListModel(Glib::RefPtr<Gtk::TreeModel> const& model, Glib::RefPtr<FilterType> const& filter)
    ...
    , matches_all_(filter->matches_all()/*whether to show all*/)
    , matches_none_(filter->matches_none()/*it is false always*/)
    , signal_changed_tag_{ filter->signal_changed().connect(
          [this, filter](auto /*Change changes*/)
          {
...
/*
    ...
    ModelT is `Gtk::TreeModel` actually
    FilterType is `TorrentFilter` who inherited from `FilterBase<Torrent>`
*/
template<typename ItemT>
