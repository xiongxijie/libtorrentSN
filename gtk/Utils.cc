...
std::string tr_format_time(std::time_t timestamp)
{
...
void gtr_add_torrent_error_dialog(Gtk::Widget& child, std::string const& duplicate_torrent_name, std::string const& filename)
{
    Glib::ustring secondary;

    if (duplicate_torrent_name.empty() != true)
    {
        secondary = fmt::format(
            _("The torrent file '{path}' is already in use by '{torrent_name}'."),
            fmt::arg("path", filename),
            fmt::arg("torrent_name", duplicate_torrent_name));
    }
    else
    {
