...
bacon_video_widget_properties_class_init (BaconVideoWidgetPropertiesClass *klass)
{
	...
	gtk_widget_class_set_template_from_resource (widget_class, "/com/transmissionbt/transmission/properties.ui");
...
bacon_video_widget_properties_reset (BaconVideoWidgetProperties *props)
{
	...
	/* Dimensions */
	bacon_video_widget_properties_set_label (props->dimensions,  "N/A");
	/* Video Codec */
	bacon_video_widget_properties_set_label (props->vcodec, "N/A");
	/* Video Bitrate */
	...
	bacon_video_widget_properties_set_label (props->audio_bitrate,
		 "N/A");
	/* Audio Codec */
	bacon_video_widget_properties_set_label (props->acodec, "N/A");
	/* Sample rate */
	bacon_video_widget_properties_set_label (props->samplerate, "0 Hz");
	/* Channels */
	bacon_video_widget_properties_set_label (props->channels, "0 Channels");
}
...
time_to_string_text (gint64 msecs)
{
	...
	hours = g_strdup_printf ("%d hour%s", hour, hour>1?"s":"");

	mins = g_strdup_printf ("%d minute%s", min, min>1?"s":"");

	secs = g_strdup_printf ("%d second%s", sec, sec>1?"s":"");
	...
	if (hour > 0)
	{
		if (min > 0 && sec > 0)
		{
			...
			/* 5 hours 2 minutes */
			string = g_strdup_printf ("%s %s", hours, mins);
		} else {
			/* 5 hours */
			string = g_strdup_printf ("%s", hours);
		}
	} else if (min > 0) {
		if (sec > 0)
		{
...
#define _24P_FPS (24000.0/1001.0)
	...
	if (framerate > 1.0) {
		int integer = (int) (ceilf (framerate));
		if (G_APPROX_VALUE (framerate, _24P_FPS, .000001)) {
			temp = g_strdup_printf ("%0.3f frame%s per second", framerate, integer>1?"s":"");
		} else {
			temp = g_strdup_printf ("%0.2f frame%s per second", framerate, integer>1?"s":"");
		}
	} else {
		temp = g_strdup ("N/A");
	}
